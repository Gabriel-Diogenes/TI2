...
public class XDAO extends DAO {
	...
	public boolean insert(X X) {
		...
		try {  
			...
			String sql = "INSERT INTO Z (codigo, login, senha, sexo) "
	...
	public boolean delete(int codigo) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "DELETE FROM Y WHERE codigo = " + codigo;
			System.out.println(sql);
			st.executeUpdate(sql);
			st.close();
			status = true;
	...
	public boolean autenticar(String login, String senha) {
		...
		try {
			Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			String sql = "SELECT * FROM Y WHERE login LIKE '" + login + "' AND senha LIKE '" + senha  + "'";
			System.out.println(sql);
			ResultSet rs = st.executeQuery(sql);
			resp = rs.next();
	        st.close();
